...
This chapter is the \emph{identical} manual from the original \emph{ClassicThesis} template by Andr\'{e} Miede.


However, you probably do not need to read the entire manual. Below are the critical steps for customizing this template:

\begin{description}
    \item[Text Content] Put your text in the \texttt{Chapters/} folder.
    
    \item[Graphics] Put your images in the \texttt{gfx/} folder.
    
    \item[Personal Information] In \texttt{classicthesis-config.tex} starting from line 54, fill in your information 
    
    \item[Configuration Options] In line 35 of \texttt{classicthesis-config.tex}, you can enable specific settings. Use \texttt{dottedtoc} to set page numbers flushed right in the Table of Contents, or enable \texttt{drafting} to print the version information on the first page of the thesis.
    
    \item[Version History] In \texttt{VersionHistory.tex}, change the version history details
\end{description}
...
\paragraph{About This Template:} This template is based on the \textit{classicthesis} package (version 4.8) by André Miede and Ivo Pletikosić. It has been modified and extended with:
\begin{itemize}
    \item APA 7th edition citation style
    \item Enhanced Japanese language support via XeLaTeX
    \item Custom Japanese author name formatting
    \item Improved multi-language (CJK) support
    \item Japanese reference sorting capabilities
\end{itemize}

These modifications are shared freely with the community under the same license as the original work.
...
\paragraph{Original classicthesis Authors' note:} There have been some discussions about the GPL's implications on using \texttt{classicthesis} for theses etc. Details can be found here:
\begin{center}
  \url{https://bitbucket.org/amiede/classicthesis/issues/123/}
\end{center}

The original authors chose the GPL to prevent proprietary modified versions. However, the template is free as free beer and free speech. Sources for theses, books, CVs, etc. created using \texttt{classicthesis} are not required to be shared. PDFs produced with \LaTeX\ and \texttt{classicthesis} are not considered derivative works in the sense of the GPL.

\paragraph{For Users:} You are free to use this template for your thesis or other academic documents. The GPL applies to the template files themselves, not to the documents you create with it. If you share or redistribute this template (especially with modifications), please maintain this license and attribution.